...
- **The browser demo's CPU half is a port and nothing checks it.** `demo/plugin.js`
  re-implements `Path.cpp`, `Controls.cpp` and the buildPath/Latch/Live parts of
  `Toolpath.cpp` in JavaScript. `check_shaders.py` covers the GLSL only. Change any of
  those and change the page by hand to match.
...
`demo/` is the page at **toolpath-demo.stoatworks-labs.com**, deployed from
`wrangler.toml` in this repo with `cf-run npx wrangler deploy` — no build step;
what is committed is what is served. `demo/vendor/` is the shared kit from
`~/Projects/infrastructure/stoatworks-backend/resolume-demo/` (plus
`support-footer/support-footer.js`) and is not a place to edit.

Verify a deploy **by content, never by status code** — a stale page answers a
cheerful 200:

    curl -s 'https://toolpath-demo.stoatworks-labs.com/?cb=1' | grep -o '<title>[^<]*'
